...
const service = {
  ...
  dataStreams: () => FIND_MANY_RESULT,
  dataStream: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("DataStream", () => {
  ...
  test("GET /dataStreams/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/dataStreams"}/${nonExistingId}`)
  ...
  test("GET /dataStreams/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/dataStreams"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /dataStreams existing resource", async () => {
